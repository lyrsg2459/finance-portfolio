...
1. Revenue growth rate (%): Most impactful uncertain assumption as most values are derived from projected revenue. How much the growth decreases YOY after FY26 is an assumption I made due to normalizing Azure capacity.  
...
## Step 3: Method to plot distribution graph from results of all simulations
`Matplotlib` is the most popular library which plots graphs and visualizations.

## Step 4: Define each function and what they should do
1. `main()`: Creates a list for each uncertain assumption, appends random values within range into each list (for 5 years)
2. `calculate_revenue(base_revenue, growth_rates)`: Creates `revenue_list`, appends base FY26 Revenue, loop 5 times with each iteration compounding by that year's growth rate, each iteration also appends into list. Return `revenue_list`. 
3. `calculate_operating_profit(revenue_list, cost_of_revenue_pct, rd_pct, sm, ga)`: Takes the new revenue list from above, calculate cost of revenue and R&D expenses, takes S&M and G&A expense (not affected by revenue so no change), finds `operating profit = c_o_r - RD - SM - GA`. Loop 5 times for each year, append each year's operating profit to `op_list` and return `op_list`.
4. `calculate_fcf(op_list, tax_rate_list, da_list, capex_list, nwc_delta_list)`: All values except operating profit are not randomised, hence just apply the formula for `NOPAT = operating_profit * (1 - tax_rate)`, and `FCF = NOPAT + D&A - Capex + ΔNWC` for each year, append to `fcf_list` and return `fcf_list`.
5. calculate_wacc():
6. calculate_terminal_value():
7. discount_cash_flows(): 
